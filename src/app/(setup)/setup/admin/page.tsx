...
export default function AdminCreationPage() {
  ...
  useEffect(() => {
    ...
    const databaseConfigured = sessionStorage.getItem("database_configured");

    if (!licenseVerified || !databaseConfigured) {
    ...
    try {
      const response = await fetch("/api/setup/create-admin", {
        ...
        body: JSON.stringify({
          ...
          mongoUri: sessionStorage.getItem("mongo_uri"),
        }),
      });
      ...
      if (response.ok) {
        ...
        sessionStorage.setItem("admin_created", "true");
        sessionStorage.setItem("admin_email", formData.email);
        router.push("/setup/store");
      } else {
        setErrors({
          general: data.message || "Failed to create admin account",
        });
      }
    } catch (error) {
      setErrors({
  ...
  return (
    <div className="space-y-8">
      <InstallationProgress currentStep={4} totalSteps={6} />
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Create your administrator account to manage your eCommerce store.
          </CardDescription>
        </CardHeader>
